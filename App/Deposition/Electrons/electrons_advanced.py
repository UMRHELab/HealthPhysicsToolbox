...
from App.Deposition.Electrons.electrons_export import electrons_export
...
def electrons_advanced(root, category, mode, interactions, common_el,
    ...
    elements = get_choices("All Elements", "Deposition", "Electrons")
    common = get_choices("Common Elements", "Deposition", "Electrons")
    non_common = [element for element in elements if element not in common]

    # Gets common and non-common materials
    materials = get_choices("All Materials", "Deposition", "Electrons")
    common_m = get_choices("Common Materials", "Deposition", "Electrons")
    non_common_m = [material for material in materials if material not in common_m]

    # Gets custom materials
    custom = get_choices("Custom Materials", "Deposition", "Electrons")
    ...
    if mode != "Radiation Yield" and mode != "Density Effect Delta":
        ...
        unit_label = ttk.Label(unit_side_frame, text=f"{mode_text} Units:",
                               style="Black.TLabel")
...
def open_ref(root):
    root.focus()
    db_path = resource_path('Utility/Modules/Deposition/Electrons/References.txt')
    open_file(db_path)
...
def open_help(root):
    root.focus()
    db_path = resource_path('Utility/Modules/Deposition/Electrons/Help.txt')
    open_file(db_path)